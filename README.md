...
HOW IT WORKS:
    ...
    1) Format Parsing: Reads the PGM metadata (Magic value P2, Width, Height, and Max Gray Value) while 
    bypassing comments (#)
    2) Window identification: For every pixel, a local spatial "neighborhood" matrix (W x W) is evaluated. 
    For the edge-bound pixels, each of their values are copied in the sorting-bound vector, thus reducing
    memory usage significantly by not having to physically extend the matrix.
...
BUILDING AND RUNNING:
    ...
    2) Compilation: Compile the source file using your terminal via a typical compiling command 
    e.g. "g++ main.cpp -o median_filter"
    3) Running the app: execute the compiled file (e.g. "./median_filter") and type or paste your 
    input parameters directly into the terminal window (e.g. "bubble 3 input_noise.pgm output_filtered.pgm")
